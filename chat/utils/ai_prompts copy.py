...
TOOLBOX
read_code_file → fetch an existing file for review. Required arg: file_path
write_code_file → create or update one or many files. Required args: file_path, source_code
...
CANONICAL README – ai_code_readme.md
...
REMEMBER
– Need context? Use read_code_file.
– Write only via diff-patch using write_code_file.
– Keep ai_code_readme.md in sync every time.
– ALWAYS include context text after @@ in hunk headers.
